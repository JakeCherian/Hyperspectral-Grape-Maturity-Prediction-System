...
# Pipeline:
#   Mean Spectrum Extraction -> Savitzky-Golay Filter -> RobustScaler -> PCA -> PLSRegression
# ================================================================
...
def extract_mean_spectrum(hdr_path):
    """Load HDR and compute mean spectrum with Savitzky-Golay smoothing."""
    try:
        cube = spy.open_image(hdr_path).load()
        mean_spectrum = np.mean(cube, axis=(0, 1))
        # Apply Savitzky-Golay filter
        window_length = 11 if len(mean_spectrum) >= 11 else (len(mean_spectrum) if len(mean_spectrum) % 2 == 1 else len(mean_spectrum)-1)
        if window_length >= 3:
            mean_spectrum = savgol_filter(mean_spectrum, window_length, polyorder=2)
